...
const GOAL_OPTIONS_MAP = {
  emergency:{emoji:"🛡️",label:"Emergency Fund",amount:2000,color:"#6EE7B7"},
  debt:     {emoji:"💳",label:"Pay Off Debt",   amount:5000,color:"#F4A9B8"},
  vacation: {emoji:"✈️",label:"Vacation Fund",  amount:3000,color:"#45B7D1"},
  home:     {emoji:"🏠",label:"Down Payment",   amount:20000,color:"#C4965A"},
  roth:     {emoji:"📈",label:"Roth IRA",       amount:7000, color:"#C4965A"},
  wedding:  {emoji:"💍",label:"Wedding Fund",   amount:15000,color:"#F4A9B8"},
  kids:     {emoji:"👶",label:"Kids / Family",  amount:10000,color:"#45B7D1"},
};

const PERSONAS = [
  { id:"nana",     name:"Nana Rose",   emoji:"👵", color:"#F4A9B8",
    react:(s,a)=>`Baby you spent $${a} at ${s}?! Nana is right here logging it with you!! 💕`,
    save:()=>"Baby you logged it!! That's half the battle!! 💕",
    resist:(s)=>`STOP!! Nana is standing in front of ${s} right now. You don't need this!! 👵🚫`,
  ...
  { id:"moneymom", name:"Money Mom",   emoji:"💁‍♀️", color:"#C4965A",
    react:(s,a)=>`$${a} at ${s}. Logged. Now let's talk about why that keeps happening.`,
    save:()=>"Logged. That's awareness. Awareness is everything.",
  ...
  { id:"savage",   name:"Savage Chef", emoji:"👨‍🍳", color:"#D4723E",
    react:(s,a)=>`$${a} at ${s}. The baristas know your name. Your Roth does not. Yet.`,
    save:()=>"Logged. Awareness precedes change. You're aware now.",
    resist:(s)=>`The ${s} can survive without you today. Your emergency fund cannot.`,
    walkaway:()=>"Walking away is a skill. You just practiced it. Do it again tomorrow.",
    badge:()=>"Badges don't lie. You earned this one. Now earn the next one.",
  },
];
...
const RR = {
  type1:{
    ...
    lines:["I've been watching your progress in Rich Girl Habits.","You're doing the right things.","Emergency fund started. Habits changing. The gap is closing.","But I see some things I'd adjust.","Small moves that compound into big differences over time.","Worth 20 minutes of your time."],
    cta:"See My Recommendations",ctaNote:"Complimentary · 20 minutes",
  },
  type2:{
    title:"It's time.",
    lines:["I'm going to be direct with you.","You've done what most people never do.","The foundation is there. The habits are there.","This is the moment where retirement planning starts.","Not when you're older. Not when you make more.","Now. At this stage. With what you have.","Let's talk."],
    cta:"Start Retirement Planning",ctaNote:"Book your complimentary call",
  },
  webinar:{
    title:"You earned an invite.",
    lines:["Not everyone gets this.","You saved $2,500.","That's every time you walked away.","Every habit you changed.","Every week you showed up.","That's not $2,500. That's you — putting yourself first.","I want to celebrate that with you."],
    cta:"Reserve My Spot",ctaNote:"Putting Yourself First · Free for Rich Girl Habits members",
...
const DEFAULT_ZONES = [
  {id:"starbucks",name:"Starbucks", emoji:"☕",weeklyGoal:4,type:"trap"},
  {id:"target",   name:"Target",   emoji:"🎯",weeklyGoal:1,type:"caution"},
  {id:"amazon",   name:"Amazon",   emoji:"📦",weeklyGoal:2,type:"trap"},
];

const FUTURE_MSGS = {
  first_log:"You just made my life a little better. I know you can't see it yet. I can.",
  first_resist:"That resistance? That's the whole game. Do it again.",
  ...
  grand:"A thousand dollars from just deciding differently. That's not luck. That's you.",
  two_five:"Twenty-five hundred dollars. You put yourself first. Maybe for the first time. I've been waiting for this.",
  identity:"You're becoming me. I can feel the gap closing from here.",
  default:"You're becoming me. I can feel the gap closing from here.",
};
...
function RRCard({type,onBook,onDismiss}){
  ...
  return(
    <div style={{background:"rgba(14,10,6,0.97)",border:"1px solid rgba(196,150,90,0.3)",borderRadius:20,padding:"28px 24px",animation:"scaleIn 0.4s ease"}}>
      ...
      <h2 style={{fontSize:22,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.cream,marginBottom:18,lineHeight:1.2}}>{data.title}</h2>
      <div style={{marginBottom:20}}>{data.lines.slice(0,idx+1).map((line,i)=><p key={i} style={{fontSize:14,color:i===idx?"rgba(253,240,232,0.9)":"rgba(253,240,232,0.5)",lineHeight:1.7,marginBottom:4,animation:"fadeUp 0.35s ease",fontWeight:i===idx?600:400}}>{line}</p>)}</div>
      {idx>=data.lines.length-1&&(
        <div style={{animation:"fadeUp 0.3s ease"}}>
          <p style={{fontSize:13,color:W.soft,fontStyle:"italic",marginBottom:type==="webinar"?12:18}}>— RetireRicher</p>
          {type==="webinar"&&<div style={{padding:"12px 14px",background:"rgba(196,150,90,0.08)",borderRadius:12,border:"1px solid rgba(196,150,90,0.15)",marginBottom:16}}><p style={{fontSize:14,fontWeight:700,color:W.cream,marginBottom:2}}>{data.event}</p><p style={{fontSize:12,color:W.soft}}>{data.eventNote}</p></div>}
          ...
          <button onClick={onDismiss} style={{width:"100%",padding:"11px",background:"transparent",border:"1px solid rgba(245,237,224,0.1)",borderRadius:12,color:W.soft,fontSize:13,cursor:"pointer"}}>I'll come back to this</button>
        </div>
      )}
    </div>
  );
...
function BadgeUnlock({badge,persona,totalSaved,onClose}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(10,5,0,0.97)",zIndex:1000,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",overflowY:"auto",fontFamily:"'Nunito',sans-serif"}}>
      ...
      {phase==="badge"&&(
        <div style={{width:"100%",maxWidth:380,textAlign:"center",animation:"scaleIn 0.4s ease"}}>
          ...
          <div style={{fontSize:80,marginBottom:16,animation:"bounceIn 0.5s ease"}}>{badge.emoji}</div>
      ...
      {phase==="future"&&(
        <div style={{width:"100%",maxWidth:380,animation:"scaleIn 0.4s ease"}}>
          ...
          <div style={{background:"rgba(196,150,90,0.08)",border:"1px solid rgba(196,150,90,0.2)",borderRadius:18,padding:"18px 20px",marginBottom:20}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}><div style={{width:36,height:36,borderRadius:"50%",background:"linear-gradient(135deg,#C4965A,#E8A87C)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>🌅</div><div><p style={{fontSize:11,fontWeight:800,color:"#C4965A",letterSpacing:0.5}}>YOUR FUTURE SELF</p><p style={{fontSize:10,color:W.soft}}>Speaking from ahead</p></div></div>
            <p style={{fontSize:14,color:"rgba(245,237,224,0.85)",lineHeight:1.7,fontStyle:"italic",fontFamily:"'Fraunces',serif",fontWeight:300}}>"{FUTURE_MSGS[badge.id]||FUTURE_MSGS.default}"</p>
          </div>
          {badge.retireRicher?<button onClick={()=>setPhase("rr")} style={{width:"100%",padding:"15px",background:"#C4965A",border:"none",borderRadius:14,color:"#0E0A06",fontSize:14,fontWeight:800,cursor:"pointer",marginBottom:10}}>RetireRicher has something to say →</button>:<button onClick={onClose} style={{width:"100%",padding:"15px",background:"#C4965A",border:"none",borderRadius:14,color:"#0E0A06",fontSize:15,fontWeight:800,cursor:"pointer",marginBottom:10}}>Keep going 🌱</button>}
          <button onClick={()=>navigator.share?.({title:`I just earned "${badge.name}" in Rich Girl Habits!`,text:`${badge.emoji} ${badge.desc}`})} style={{width:"100%",padding:"12px",background:"transparent",border:"1px solid rgba(245,237,224,0.12)",borderRadius:12,color:W.soft,fontSize:13,cursor:"pointer"}}>Share this moment ↗</button>
        </div>
      )}
...
function Bodyguard({zone,persona,resistanceStreak,onWalkAway,onWentIn}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(10,5,0,0.97)",zIndex:1000,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",fontFamily:"'Nunito',sans-serif"}}>
      {phase==="intercept"&&(
        <div style={{width:"100%",maxWidth:380,animation:"scaleIn 0.35s ease"}}>
          <div style={{textAlign:"center",marginBottom:24}}>
            <div style={{fontSize:56,marginBottom:12,animation:"pulse 1.5s infinite"}}>🚨</div>
            <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(239,68,68,0.12)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:99,padding:"5px 14px",marginBottom:16}}><div style={{width:8,height:8,borderRadius:"50%",background:"#EF4444",animation:"blink 1s infinite"}}/><span style={{fontSize:11,fontWeight:800,color:"#FCA5A5",letterSpacing:1.5}}>BUDGET BODYGUARD</span></div>
            <h2 style={{fontSize:26,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.cream,lineHeight:1.2}}>{zone.emoji} {zone.name} detected</h2>
          </div>
          <div style={{background:"rgba(244,169,184,0.1)",border:"1px solid rgba(244,169,184,0.2)",borderRadius:14,padding:"14px 16px",marginBottom:20}}>
            <div style={{display:"flex",gap:10,alignItems:"flex-start"}}><span style={{fontSize:24,flexShrink:0}}>{persona.emoji}</span><div><p style={{fontSize:11,fontWeight:800,color:persona.color,marginBottom:4}}>{persona.name}</p><p style={{fontSize:13,color:W.mid,lineHeight:1.65}}>{persona.resist(zone.name)}</p></div></div>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            <button onClick={()=>setPhase("followup")} style={{padding:"16px",background:"#C4965A",border:"none",borderRadius:14,color:"#0E0A06",fontSize:15,fontWeight:800,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10}}><span style={{fontSize:20}}>💪</span> Walking Away</button>
            <button onClick={onWentIn} style={{padding:"14px",background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.25)",borderRadius:14,color:"#FCA5A5",fontSize:14,fontWeight:700,cursor:"pointer"}}>I'm going in</button>
          </div>
          <p style={{fontSize:11,color:W.soft,textAlign:"center",marginTop:12}}>Your future self wants you to keep walking.</p>
        </div>
      )}
      {phase==="followup"&&(
        <div style={{width:"100%",maxWidth:380,textAlign:"center",animation:"scaleIn 0.35s ease"}}>
          ...
          <p style={{fontSize:14,color:W.soft,marginBottom:24,lineHeight:1.6}}>Did you actually walk away from {zone.emoji} {zone.name}?</p>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
...
function WalkAwayResult({persona,resistanceStreak,onClose}){
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(10,5,0,0.97)",zIndex:1000,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",fontFamily:"'Nunito',sans-serif"}}>
      <Confetti active={true}/>
      <div style={{width:"100%",maxWidth:380,textAlign:"center",animation:"scaleIn 0.35s ease"}}>
        <div style={{fontSize:64,marginBottom:16}}>🏆</div>
        <h2 style={{fontSize:28,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.cream,marginBottom:8}}>You walked away.</h2>
        ...
        {resistanceStreak>=3&&<div style={{background:"rgba(251,191,36,0.08)",border:"1px solid rgba(251,191,36,0.2)",borderRadius:12,padding:"10px 14px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:20}}>🔥</span><p style={{fontSize:12,fontWeight:700,color:"#FDE68A"}}>{resistanceStreak} resistances in a row!</p></div>}
        <button style={{width:"100%",padding:"15px",background:"#C4965A",border:"none",borderRadius:14,color:"#0E0A06",fontSize:14,fontWeight:800,cursor:"pointer",marginBottom:10}}>💰 Secure The Bag → Move $8.50</button>
...
function Onboarding({onComplete}){
  ...
  const [members,setMembers]=useState([{name:"Me",emoji:"👩"}]);
  ...
  const GOAL_OPTIONS=[
    {id:"emergency",emoji:"🛡️",label:"Emergency Fund",amount:2000},
    {id:"debt",emoji:"💳",label:"Pay Off Debt",amount:5000},
    {id:"vacation",emoji:"✈️",label:"Vacation Fund",amount:3000},
    {id:"home",emoji:"🏠",label:"Down Payment",amount:20000},
    {id:"roth",emoji:"📈",label:"Roth IRA",amount:7000},
    {id:"wedding",emoji:"💍",label:"Wedding Fund",amount:15000},
    {id:"kids",emoji:"👶",label:"Kids / Family",amount:10000},
  ];
  const ACCTS=["Checking account","Savings account","401k or 403b","Roth IRA","Brokerage account"];
  ...
  const btn=(s={})=>({width:"100%",padding:"15px",background:W.accent,border:"none",borderRadius:14,color:W.bg,fontSize:15,fontWeight:800,cursor:"pointer",fontFamily:"'Nunito',sans-serif",...s});
  ...
  const SCREENS=[
    ...
    <div key={0} style={{minHeight:"100vh",background:W.bg,backgroundImage:"url('/7EA228FB-BB36-4BE9-829D-C86EAAE98B1F.png')",backgroundSize:"cover",backgroundPosition:"center",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 24px",textAlign:"center"}}>
      <h1 style={{fontSize:42,fontWeight:700,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:"#8B4513",marginBottom:12,lineHeight:1.1}}>Rich Girl Habits</h1>
      <p style={{fontSize:18,color:"#5C3317",marginBottom:8,lineHeight:1.6,fontStyle:"italic",fontFamily:"'Fraunces',serif",fontWeight:600}}>"Your future self wants you to."</p>
      <p style={{fontSize:15,color:"#5C3317",marginBottom:48,fontWeight:600}}>"For when you have more month than money."</p>
    ...
    <div key={2} style={{minHeight:"100vh",background:W.bg,padding:"60px 24px 100px",fontFamily:"'Nunito',sans-serif"}}>
      <h2 style={{fontSize:24,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.cream,marginBottom:8,textAlign:"center"}}>Here's how Rich Girl Habits works</h2>
      <p style={{fontSize:13,color:W.soft,textAlign:"center",marginBottom:28}}>Four things that change everything</p>
      {[{emoji:"🎙️",title:"Voice logging",desc:"Say 'I spent $38 at Starbucks' — AI parses it instantly."},{emoji:"👩‍💼",title:"Your coach",desc:"Pick a persona who reacts to everything you do. In character. Always."},{emoji:"📧",title:"Sunday Report",desc:"Every Sunday — your week in numbers, your pattern, your goal, what it becomes."},{emoji:"🌅",title:"Future Self",desc:"She speaks to you at every milestone. Reminding you why you started."}].map(c=>(
        <div key={c.emoji} style={{background:W.card,border:`1px solid ${W.border}`,borderRadius:18,padding:18,marginBottom:12,display:"flex",gap:14,alignItems:"flex-start"}}>
          <span style={{fontSize:28,flexShrink:0}}>{c.emoji}</span>
          <div><p style={{fontSize:14,fontWeight:700,marginBottom:4,color:W.cream}}>{c.title}</p><p style={{fontSize:13,color:W.mid,lineHeight:1.6}}>{c.desc}</p></div>
        </div>
      ))}
      <button onClick={next} style={btn({marginTop:16})}>I'm ready →</button>
    </div>,
    // 3 name
    <div key={3} style={{minHeight:"100vh",background:W.bg,padding:"60px 24px 100px",fontFamily:"'Nunito',sans-serif"}}>
      {isGenius&&<div style={{background:W.card,border:`1px solid ${W.border}`,borderRadius:18,padding:18,marginBottom:20,display:"flex",gap:10,alignItems:"flex-start"}}><span style={{fontSize:22}}>😏</span><p style={{fontSize:13,color:W.mid,lineHeight:1.6,fontStyle:"italic"}}>"We'll be here when you have more month than money."</p></div>}
      <h2 style={{fontSize:28,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.cream,marginBottom:24}}>First — what do we call you?</h2>
      {[["Name",name,setName,"Your first name"],["Phone",phone,setPhone,"For Budget Bodyguard alerts"],["Email",email,setEmail,"For your Sunday Report"]].map(([label,val,setter,ph])=>(
        <div key={label} style={{marginBottom:14}}>
          <p style={{fontSize:11,color:W.soft,letterSpacing:1,textTransform:"uppercase",marginBottom:6}}>{label}</p>
          <input value={val} onChange={e=>setter(e.target.value)} placeholder={ph} style={{width:"100%",padding:"14px",background:"rgba(255,255,255,0.05)",border:`1px solid ${W.border}`,borderRadius:12,color:W.cream,fontSize:14,outline:"none"}}/>
        </div>
      ))}
    ...
    <div key={6} style={{minHeight:"100vh",background:W.bg,padding:"60px 24px 100px",fontFamily:"'Nunito',sans-serif"}}>
      <h2 style={{fontSize:24,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.cream,marginBottom:8}}>What do you currently have?</h2>
      <p style={{fontSize:13,color:W.soft,marginBottom:24}}>Check all that apply</p>
      {ACCTS.map(a=>{
        const sel=accounts.includes(a);
    ...
    <div key={7} style={{minHeight:"100vh",background:W.bg,padding:"60px 24px 100px",fontFamily:"'Nunito',sans-serif"}}>
      ...
      {PERSONAS.map(p=>{
        ...
        return(<button key={p.id} onClick={()=>setSelectedPersona(p)} style={{width:"100%",padding:"14px 16px",background:sel?W.aLight:"rgba(255,255,255,0.03)",border:`1.5px solid ${sel?W.accent:W.border}`,borderRadius:14,marginBottom:10,cursor:"pointer",display:"flex",alignItems:"center",gap:12,fontFamily:"'Nunito',sans-serif",transition:"all 0.2s"}}>
          ...
          <div style={{textAlign:"left",flex:1}}><p style={{fontSize:14,fontWeight:700,color:sel?W.cream:W.mid}}>{p.name}</p><p style={{fontSize:12,color:W.soft,marginTop:2,fontStyle:"italic"}}>{p.react("Starbucks","38.50").slice(0,55)}...</p></div>
          {sel&&<span style={{color:W.accent,fontSize:18}}>✓</span>}
        </button>);
      })}
  ...
  return(
    <div style={{fontFamily:"'Nunito',sans-serif",color:W.cream}}>
      ...
      {screen>0&&<div style={{position:"fixed",bottom:40,left:"50%",transform:"translateX(-50%)",display:"flex",gap:4,zIndex:100}}>{SCREENS.map((_,i)=>i>0&&<div key={i} style={{width:i===screen?20:6,height:6,borderRadius:3,background:i===screen?W.accent:"rgba(253,240,232,0.15)",transition:"all 0.3s"}}/>)}</div>}
    </div>
  );
}
...
export default function App(){
  ...
  const userGoals=goals.length>0?goals:[];
  ...
  const totalSaved=userGoals.reduce((s,g)=>s+g.saved,0);
  const byCat={};
  ...
  const demo=()=>{
    ...
    setParsed({amount:38.50,store:"Starbucks",date:todayStr(),purpose:"coffee",category:"food"});
  };

  const save=()=>{
    if(!parsed)return;
    setExpenses(p=>[{id:nextId.current++,...parsed,member:user?.name||"You"},...p]);
    setSaving(true);
  ...
  if(!onboarded) return(
    ...
    <Onboarding onComplete={(data)=>{
      setUser(data);
      setGoals(data.goals.map(g=>{const gd=GOAL_OPTIONS_MAP[g]||{emoji:"🎯",label:g,amount:1000,color:"#6EE7B7"};return{id:g,...gd,saved:0};}));
      setOnboarded(true);
    }}/></>
  );
  ...
  return(
    <div style={{minHeight:"100vh",background:W.bg,fontFamily:"'Nunito',sans-serif",color:W.text,maxWidth:430,margin:"0 auto"}}>
      ...
      {showWalkAway&&<WalkAwayResult persona={persona} resistanceStreak={resistanceStreak+1} onClose={()=>{setShowWalkAway(false);setActiveBodyguard(null);setResistanceStreak(s=>s+1);checkBadge("first_resist");}}/>}
      ...
      {tab==="home"&&(
        <div style={{padding:"0 20px 120px",animation:"scaleIn 0.35s ease"}}>
          {reaction&&(
            <div style={{...card({background:"rgba(244,169,184,0.1)",borderColor:"rgba(244,169,184,0.25)",marginBottom:14,animation:"fadeUp 0.4s ease",display:"flex",gap:10,alignItems:"flex-start"})}}>
              <span style={{fontSize:22,animation:"float 2s ease-in-out infinite"}}>{persona.emoji}</span>
          ...
          <p style={{fontSize:10,color:W.soft,letterSpacing:1.5,textTransform:"uppercase",marginBottom:10}}>Your Goals</p>
          {userGoals.length===0?(
            <div style={{...card({textAlign:"center",padding:"24px 20px"})}}>
              <p style={{fontSize:32,marginBottom:8}}>🎯</p>
              <p style={{fontSize:13,color:W.soft}}>Complete onboarding to set your goals</p>
            </div>
          ):userGoals.map(g=>(
            <div key={g.id} style={{...card({marginBottom:10})}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:20}}>{g.emoji}</span><div><p style={{fontSize:13,fontWeight:700}}>{g.label}</p><p style={{fontSize:11,color:W.soft}}>${(g.target-g.saved).toLocaleString()} to go</p></div></div>
          ...
          <p style={{fontSize:10,color:W.soft,letterSpacing:1.5,textTransform:"uppercase",marginBottom:10,marginTop:6}}>Priority Ladder</p>
          {LADDER.map(l=>(
            <div key={l.step} style={{...card({marginBottom:8,display:"flex",gap:12,alignItems:"center"})}}>
              <div style={{width:32,height:32,borderRadius:"50%",background:l.step===1?"rgba(110,231,183,0.15)":"rgba(255,255,255,0.05)",border:`1.5px solid ${l.step===1?"#6EE7B7":W.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0,fontWeight:700,color:l.step===1?"#6EE7B7":W.soft}}>{l.step===1?"✓":l.step}</div>
              <div style={{flex:1}}><p style={{fontSize:13,fontWeight:700,color:l.step===1?"#6EE7B7":W.cream}}>{l.emoji} {l.name}</p><p style={{fontSize:11,color:W.soft}}>{l.desc}</p></div>
            </div>
          ))}
        </div>
      )}
      ...
      {tab==="record"&&(
        <div style={{padding:"0 20px 120px",animation:"scaleIn 0.35s ease"}}>
          {reaction&&(
            <div style={{...card({background:"rgba(244,169,184,0.1)",borderColor:"rgba(244,169,184,0.25)",marginBottom:14,animation:"fadeUp 0.4s ease",display:"flex",gap:10})}}>
          ...
          <button onClick={demo} style={{width:"100%",padding:"11px",background:"transparent",border:`1px dashed ${W.border}`,borderRadius:12,color:W.soft,fontSize:12,cursor:"pointer",marginBottom:14}}>✨ Try demo input</button>
          {transcript&&<div style={{...card({marginBottom:14}),animation:"fadeUp 0.3s ease"}}><p style={{fontSize:10,color:W.soft,marginBottom:6,textTransform:"uppercase",letterSpacing:1}}>Heard</p><p style={{fontSize:14,fontStyle:"italic",color:W.mid}}>"{transcript}"</p></div>}
          {parsed&&!saving&&(
      ...
      {tab==="growth"&&(
        <div style={{padding:"0 20px 120px",animation:"scaleIn 0.35s ease"}}>
          <div style={{...card({background:"rgba(232,149,106,0.08)",borderColor:`${W.accent}33`,marginBottom:16,padding:"24px 20px",textAlign:"center"})}}>
            ...
            <p style={{fontSize:13,color:W.mid,marginTop:6,fontStyle:"italic",fontFamily:"'Fraunces',serif"}}>"Your future self is getting closer."</p>
            {totalSaved>=2500&&<div style={{marginTop:12,padding:"8px 14px",background:"rgba(232,149,106,0.1)",borderRadius:10,display:"inline-block"}}><p style={{fontSize:12,color:W.mid}}>🎓 Webinar invite: <span style={{color:W.accent,fontWeight:800}}>You've earned it!</span></p></div>}
          </div>
          ...
          <div style={{...card({marginBottom:14})}}>
            ...
            <div style={{marginTop:14,padding:"14px",background:"rgba(232,149,106,0.12)",borderRadius:12,border:"1px solid rgba(232,149,106,0.25)",textAlign:"center"}}>
              <Sparkle size={16}/>
              <p style={{fontSize:13,color:W.mid,marginBottom:4,marginTop:8}}>In 5 years that's</p>
          ...
          <div style={{...card({background:"rgba(232,149,106,0.06)",display:"flex",gap:10})}}>
            ...
            <div><p style={{fontSize:12,fontWeight:800,color:W.accent,marginBottom:4}}>RetireRicher</p><p style={{fontSize:12,color:W.mid,lineHeight:1.65,fontStyle:"italic"}}>"Know where you are. Know where you want to be. Identify the gap. That's the whole game — and you're playing it."</p></div>
          </div>
        </div>
      )}
