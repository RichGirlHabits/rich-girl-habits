...
const BADGES = [
  { id:"first_log",   emoji:"🌱", name:"First Seed",      desc:"Logged your first expense",     tier:"starter" },
  { id:"first_resist",emoji:"💪", name:"The Wall",        desc:"Resisted a Danger Zone",         tier:"bronze"  },
...
const LADDER = [
  {step:1,emoji:"🛡️",name:"Emergency Fund",   desc:"1 month expenses saved"},
...
const FUTURE_MSGS = {
  first_log:"You just made my life a little better. I know you cannot see it yet. I can.",
  first_resist:"That resistance? That is the whole game. Do it again.",
...
// Local storage helpers for login persistence
function saveUser(data){try{localStorage.setItem("rgh_user",JSON.stringify(data));}catch(e){}}
function loadUser(){try{const d=localStorage.getItem("rgh_user");return d?JSON.parse(d):null;}catch(e){return null;}}
function saveExpenses(data){try{localStorage.setItem("rgh_expenses",JSON.stringify(data));}catch(e){}}
function loadExpenses(){try{const d=localStorage.getItem("rgh_expenses");return d?JSON.parse(d):[];}catch(e){return[];}}
  ...
  return(
    <div style={{background:"rgba(14,10,6,0.97)",border:"1px solid rgba(196,150,90,0.3)",borderRadius:20,padding:"28px 24px"}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
        ...
        <div><p style={{fontSize:14,fontWeight:800,color:"#C4965A",letterSpacing:1}}>RETIREDRICHER</p><p style={{fontSize:13,color:W.soft}}>A personal message</p></div>
      </div>
      <h2 style={{fontSize:26,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.cream,marginBottom:18}}>{data.title}</h2>
      ...
      {idx>=data.lines.length-1&&(
        <div>
          <p style={{fontSize:16,color:W.soft,fontStyle:"italic",marginBottom:14}}>— RetireRicher</p>
          {type==="webinar"&&<div style={{padding:"12px 14px",background:"rgba(196,150,90,0.08)",borderRadius:12,marginBottom:16}}><p style={{fontSize:14,fontWeight:700,color:W.cream,marginBottom:2}}>{data.event}</p><p style={{fontSize:14,color:W.soft}}>{data.eventNote}</p></div>}
          ...
          <p style={{fontSize:13,color:W.soft,textAlign:"center",marginBottom:10}}>{data.ctaNote}</p>
          <button onClick={onDismiss} style={{width:"100%",padding:"11px",background:"transparent",border:"1px solid rgba(245,237,224,0.1)",borderRadius:12,color:W.soft,fontSize:16,cursor:"pointer"}}>I will come back to this</button>
        </div>
      )}
    </div>
  );
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(10,5,0,0.97)",zIndex:1000,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",overflowY:"auto",fontFamily:"'Nunito',sans-serif"}}>
      ...
      {phase==="badge"&&(
        <div style={{width:"100%",maxWidth:380,textAlign:"center"}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(255,255,255,0.06)",borderRadius:99,padding:"4px 14px",marginBottom:24}}><div style={{width:8,height:8,borderRadius:"50%",background:tc}}/><span style={{fontSize:13,fontWeight:800,color:tc,letterSpacing:1.5,textTransform:"uppercase"}}>{badge.tier}</span></div>
          ...
          <div style={{background:"rgba(196,150,90,0.1)",border:"1px solid rgba(196,150,90,0.2)",borderRadius:14,padding:"14px",marginBottom:20,display:"inline-block",minWidth:200}}>
            <p style={{fontSize:13,color:W.soft,marginBottom:4,textTransform:"uppercase",letterSpacing:1}}>Total Saved</p>
          ...
          <div style={{background:"rgba(244,169,184,0.1)",border:"1px solid rgba(244,169,184,0.2)",borderRadius:14,padding:"14px 16px",marginBottom:24,textAlign:"left"}}>
            <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
              ...
              <div><p style={{fontSize:13,fontWeight:800,color:persona.color,marginBottom:4}}>{persona.name}</p><p style={{fontSize:16,color:W.mid,lineHeight:1.6}}>{persona.badge()}</p></div>
            </div>
          </div>
      ...
      {phase==="future"&&(
        <div style={{width:"100%",maxWidth:380}}>
          <div style={{textAlign:"center",marginBottom:24}}><span style={{fontSize:52}}>{badge.emoji}</span><h2 style={{fontSize:26,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.cream,marginTop:10}}>{badge.name}</h2></div>
          <div style={{background:"rgba(196,150,90,0.08)",border:"1px solid rgba(196,150,90,0.2)",borderRadius:18,padding:"18px 20px",marginBottom:20}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}><div style={{width:36,height:36,borderRadius:"50%",background:"linear-gradient(135deg,#C4965A,#E8A87C)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>🌅</div><p style={{fontSize:13,fontWeight:800,color:"#C4965A",letterSpacing:0.5}}>YOUR FUTURE SELF</p></div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(10,5,0,0.97)",zIndex:1000,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",fontFamily:"'Nunito',sans-serif"}}>
      {phase==="intercept"&&(
        <div style={{width:"100%",maxWidth:380}}>
          <div style={{textAlign:"center",marginBottom:24}}>
            ...
            <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(239,68,68,0.12)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:99,padding:"5px 14px",marginBottom:16}}><div style={{width:8,height:8,borderRadius:"50%",background:"#EF4444"}}/><span style={{fontSize:13,fontWeight:800,color:"#FCA5A5",letterSpacing:1.5}}>BUDGET BODYGUARD</span></div>
          ...
          <div style={{background:"rgba(244,169,184,0.1)",border:"1px solid rgba(244,169,184,0.2)",borderRadius:14,padding:"14px 16px",marginBottom:20}}>
            <div style={{display:"flex",gap:10}}><span style={{fontSize:24,flexShrink:0}}>{persona.emoji}</span><div><p style={{fontSize:13,fontWeight:800,color:persona.color,marginBottom:4}}>{persona.name}</p><p style={{fontSize:16,color:W.mid,lineHeight:1.65}}>{persona.resist(zone.name)}</p></div></div>
          </div>
      ...
      {phase==="followup"&&(
        <div style={{width:"100%",maxWidth:380,textAlign:"center"}}>
          ...
          <h2 style={{fontSize:28,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.cream,marginBottom:8}}>10 minutes later...</h2>
          <p style={{fontSize:14,color:W.soft,marginBottom:24}}>Did you actually walk away from {zone.emoji} {zone.name}?</p>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            ...
            <button onClick={onWentIn} style={{padding:"14px",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:14,color:W.soft,fontSize:16,fontWeight:600,cursor:"pointer"}}>Okay I went in</button>
          </div>
        </div>
      )}
    </div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(10,5,0,0.97)",zIndex:1000,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",fontFamily:"'Nunito',sans-serif"}}>
      ...
      <div style={{width:"100%",maxWidth:380,textAlign:"center"}}>
        ...
        <div style={{background:"rgba(196,150,90,0.1)",border:"1px solid rgba(196,150,90,0.25)",borderRadius:14,padding:"16px",marginBottom:20,display:"inline-block",minWidth:220}}>
          <p style={{fontSize:13,color:W.soft,marginBottom:4,textTransform:"uppercase",letterSpacing:1}}>Saved Just Now</p>
        ...
        <div style={{background:"rgba(244,169,184,0.1)",border:"1px solid rgba(244,169,184,0.2)",borderRadius:14,padding:"14px 16px",marginBottom:16,textAlign:"left"}}>
          <div style={{display:"flex",gap:10}}><span style={{fontSize:22,flexShrink:0}}>{persona.emoji}</span><p style={{fontSize:16,color:W.mid,lineHeight:1.65}}>{persona.walkaway()}</p></div>
        </div>
        ...
        <button onClick={onClose} style={{width:"100%",padding:"12px",background:"transparent",border:"1px solid rgba(245,237,224,0.1)",borderRadius:12,color:W.soft,fontSize:16,cursor:"pointer"}}>Done</button>
      </div>
    </div>
  );
}
...
function Login({onLogin, onRegister}){
  ...
  const handleLogin=()=>{
    ...
    if(!saved){setError("No account found. Please register.");return;}
    if(saved.email===email){onLogin(saved);}
    else{setError("Email not found. Please register.");}
  };
  ...
  const btn2={width:"100%",padding:"15px",background:W.accent,border:"none",borderRadius:14,color:W.bg,fontSize:18,fontWeight:800,cursor:"pointer",fontFamily:"'Nunito',sans-serif"};
  ...
  return(
    <div style={{minHeight:"100vh",background:W.bg,backgroundImage:"url('/7EA228FB-BB36-4BE9-829D-C86EAAE98B1F.png')",backgroundSize:"cover",backgroundPosition:"center",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 24px",fontFamily:"'Nunito',sans-serif"}}>
      ...
      <h1 style={{fontSize:36,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.accent,marginBottom:6,textAlign:"center"}}>Rich Girl Habits</h1>
      <p style={{fontSize:16,color:W.mid,marginBottom:32,textAlign:"center",fontStyle:"italic"}}>"Your future self wants you to."</p>
      <div style={{width:"100%",maxWidth:380,background:"rgba(10,5,0,0.85)",borderRadius:20,padding:"28px 24px",border:`1px solid ${W.border}`}}>
        <div style={{display:"flex",gap:4,marginBottom:24}}>
          <button onClick={()=>{setMode("login");setError("");}} style={{flex:1,padding:"10px",borderRadius:10,border:"none",cursor:"pointer",fontSize:16,fontWeight:800,background:mode==="login"?W.accent:"rgba(255,255,255,0.05)",color:mode==="login"?W.bg:W.soft,fontFamily:"'Nunito',sans-serif"}}>Sign In</button>
          <button onClick={()=>{setMode("register");setError("");}} style={{flex:1,padding:"10px",borderRadius:10,border:"none",cursor:"pointer",fontSize:16,fontWeight:800,background:mode==="register"?W.accent:"rgba(255,255,255,0.05)",color:mode==="register"?W.bg:W.soft,fontFamily:"'Nunito',sans-serif"}}>Register</button>
        </div>
        {mode==="register"&&<input value={name} onChange={e=>setName(e.target.value)} placeholder="Your first name" style={inp}/>}
        ...
        {mode==="register"&&<input value={zip} onChange={e=>setZip(e.target.value)} placeholder="ZIP code (for local insights)" style={inp}/>}
        {error&&<p style={{fontSize:14,color:"#FCA5A5",marginBottom:12,textAlign:"center"}}>{error}</p>}
        {mode==="login"?(
          <button onClick={handleLogin} style={btn2}>Sign In →</button>
        ):(
          <button onClick={()=>{if(!name||!email){setError("Please enter your name and email.");return;}onRegister({name,email,password,zip});}} style={btn2}>Create Account →</button>
        )}
      </div>
      <p style={{fontSize:13,color:W.soft,marginTop:16,textAlign:"center"}}>A RetireRicher product</p>
    </div>
  );
}
...
function Onboarding({user, onComplete}){
  ...
  const btn=(s={})=>({width:"100%",padding:"15px",background:W.accent,border:"none",borderRadius:14,color:W.bg,fontSize:18,fontWeight:800,cursor:"pointer",fontFamily:"'Nunito',sans-serif",...s});

  const startRec=()=>{
    setTranscript("");
    if(!("webkitSpeechRecognition" in window)&&!("SpeechRecognition" in window)){
      setTranscript("Demo: I spent $38 at Starbucks for coffee");
      setFirstExpense({amount:38,store:"Starbucks",date:todayStr(),purpose:"coffee",category:"food",member:user.name});
      return;
    }
    ...
    rec.onend=()=>{setRecording(false);if(last){setFirstExpense({amount:38,store:"Starbucks",date:todayStr(),purpose:"coffee",category:"food",member:user.name});}};
  ...
  const SCREENS=[
    // 0 — Explainer (skip if genius)
    <div key={0} style={{minHeight:"100vh",background:W.bg,padding:"60px 24px 100px",fontFamily:"'Nunito',sans-serif"}}>
      <h2 style={{fontSize:28,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.cream,marginBottom:8,textAlign:"center"}}>Here is how Rich Girl Habits works</h2>
      <p style={{fontSize:16,color:W.soft,textAlign:"center",marginBottom:28}}>Four things that change everything</p>
      {[
        {emoji:"🎙️",title:"Voice logging",desc:"Say 'I spent $38 at Starbucks' — AI parses it instantly."},
        ...
        <div key={item.emoji} style={{...c({marginBottom:12,display:"flex",gap:14,alignItems:"flex-start"})}}>
          ...
          <div><p style={{fontSize:14,fontWeight:700,marginBottom:4,color:W.cream}}>{item.title}</p><p style={{fontSize:16,color:W.mid,lineHeight:1.6}}>{item.desc}</p></div>
        </div>
      ))}
      <button onClick={next} style={btn({marginTop:16})}>I am ready →</button>
    </div>,

    // 1 — Tell me about your habits
    <div key={1} style={{minHeight:"100vh",background:W.bg,display:"flex",flexDirection:"column",justifyContent:"center",padding:"40px 24px"}}>
      ...
      {[
        {emoji:"💸",label:"I spend first, save later",desc:"Whatever is left at the end of the month... if there is anything left",genius:false},
        {emoji:"📊",label:"I track but I am inconsistent",desc:"I start strong then fall off. I know what I should do, I just do not do it",genius:false},
        {emoji:"🌱",label:"I am building good habits",desc:"I am getting there. I save regularly but want to do more",genius:false},
        {emoji:"💎",label:"I have got this",desc:"My habits are solid. I just want to track and grow",genius:true},
      ].map((opt,i)=>(
        <button key={i} onClick={()=>{setHabitLabel(opt.label);setIsGenius(opt.genius);next();}} style={{width:"100%",padding:"16px 18px",background:W.card,border:`1.5px solid ${W.border}`,borderRadius:14,marginBottom:10,cursor:"pointer",display:"flex",alignItems:"center",gap:12,fontFamily:"'Nunito',sans-serif",textAlign:"left",transition:"all 0.2s"}}>
          <span style={{fontSize:28,flexShrink:0}}>{opt.emoji}</span>
          <div><p style={{fontSize:15,fontWeight:700,color:W.cream,marginBottom:3}}>{opt.label}</p><p style={{fontSize:14,color:W.soft}}>{opt.desc}</p></div>
        </button>
      ))}
    </div>,

    // 2 — Goals + preset amounts
    <div key={2} style={{minHeight:"100vh",background:W.bg,padding:"60px 24px 100px",fontFamily:"'Nunito',sans-serif"}}>
      ...
      <p style={{fontSize:16,color:W.soft,marginBottom:24}}>Select goals and tap an amount</p>
        ...
        return(
          <div key={id} style={{marginBottom:8}}>
            <button onClick={()=>setSelectedGoals(p=>sel?p.filter(x=>x!==id):[...p,id])} style={{width:"100%",padding:"14px 16px",background:sel?W.aLight:"rgba(255,255,255,0.03)",border:`1.5px solid ${sel?W.accent:W.border}`,borderRadius:sel?"14px 14px 0 0":14,color:sel?W.cream:W.mid,cursor:"pointer",display:"flex",alignItems:"center",gap:12,fontFamily:"'Nunito',sans-serif",transition:"all 0.2s"}}>
              ...
              <div style={{textAlign:"left",flex:1}}><p style={{fontSize:14,fontWeight:700}}>{g.label}</p>{chosenAmt&&<p style={{fontSize:13,color:"#6EE7B7",marginTop:2}}>Target: ${chosenAmt.toLocaleString()}</p>}</div>
              <span style={{color:sel?W.accent:W.soft,fontSize:16}}>{sel?"▲":"▼"}</span>
            </button>
            {sel&&(
              <div style={{padding:"12px 14px",background:"rgba(255,255,255,0.02)",borderRadius:"0 0 14px 14px",border:`1px solid ${W.border}`,borderTop:"none"}}>
                <p style={{fontSize:13,color:W.soft,marginBottom:8}}>Tap your target amount:</p>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  {g.presets.map(p=>(
                    <button key={p} onClick={()=>setGoalAmounts(prev=>({...prev,[id]:p}))} style={{padding:"8px 14px",background:chosenAmt===p?"#C4965A":W.aLight,border:`1px solid ${chosenAmt===p?"#C4965A":W.border}`,borderRadius:99,fontSize:16,color:chosenAmt===p?"#0A0500":W.cream,fontWeight:700,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>${p.toLocaleString()}</button>
                  ))}
                  <button onClick={()=>{const amt=prompt("Enter your target amount:");if(amt&&!isNaN(amt))setGoalAmounts(prev=>({...prev,[id]:parseInt(amt)}));}} style={{padding:"8px 14px",background:"transparent",border:`1px dashed ${W.border}`,borderRadius:99,fontSize:16,color:W.soft,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>Custom</button>
                </div>
              </div>
            )}
          </div>
    ...
    // 3 — Weekly savings — SELECTABLE
    <div key={3} style={{minHeight:"100vh",background:W.bg,padding:"60px 24px 100px",fontFamily:"'Nunito',sans-serif"}}>
      <h2 style={{fontSize:28,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.cream,marginBottom:8}}>How much can you save weekly?</h2>
      <p style={{fontSize:16,color:W.soft,marginBottom:24}}>Tap to select your commitment</p>
      {[10,25,50,100].map(amt=>{
        const y5=compound(amt,5);
        const sel=weeklyCommitment===amt;
        return(
          <button key={amt} onClick={()=>setWeeklyCommitment(amt)} style={{width:"100%",padding:"18px",background:sel?W.aLight:"rgba(255,255,255,0.03)",border:`2px solid ${sel?W.accent:W.border}`,borderRadius:14,marginBottom:10,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",fontFamily:"'Nunito',sans-serif",transition:"all 0.2s"}}>
            <div style={{textAlign:"left"}}>
              <p style={{fontSize:18,fontWeight:800,color:sel?W.cream:W.accent}}>${amt}/week</p>
              <p style={{fontSize:13,color:W.soft}}>${(amt*52).toLocaleString()}/year</p>
            </div>
            <div style={{textAlign:"right"}}>
              <p style={{fontSize:15,fontWeight:700,color:"#6EE7B7"}}>{formatK(y5)} in 5 years</p>
              <p style={{fontSize:13,color:W.soft}}>at 7% return</p>
            </div>
            {sel&&<div style={{position:"absolute",right:24,top:"50%",transform:"translateY(-50%)",width:20,height:20,borderRadius:"50%",background:W.accent,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,color:W.bg,fontWeight:800}}>✓</div>}
          </button>
        );
      })}
      <div style={{...c({background:"rgba(232,149,106,0.08)",marginTop:4,marginBottom:20,textAlign:"center"})}}>
        <Sparkle size={14}/>
        <p style={{fontSize:14,color:W.mid,marginTop:8,lineHeight:1.6}}><strong style={{color:W.accent}}>$10/week</strong> from cutting one coffee becomes <strong style={{color:"#6EE7B7"}}>{formatK(compound(10,30))}</strong> in 30 years.</p>
      </div>
      <button onClick={next} disabled={!weeklyCommitment} style={btn({opacity:weeklyCommitment?1:0.4})}>That is powerful →</button>
    </div>,

    // 5 — Choose your coach
    <div key={5} style={{minHeight:"100vh",background:W.bg,padding:"60px 24px 100px",fontFamily:"'Nunito',sans-serif"}}>
      <h2 style={{fontSize:28,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.cream,marginBottom:8}}>Choose Your Coach</h2>
      <p style={{fontSize:16,color:W.soft,marginBottom:24}}>Who do you want in your corner?</p>
      {PERSONAS.map(p=>{
        const sel=selectedPersona?.id===p.id;
        return(<button key={p.id} onClick={()=>setSelectedPersona(p)} style={{width:"100%",padding:"14px 16px",background:sel?W.aLight:"rgba(255,255,255,0.03)",border:`1.5px solid ${sel?W.accent:W.border}`,borderRadius:14,marginBottom:10,cursor:"pointer",display:"flex",alignItems:"center",gap:12,fontFamily:"'Nunito',sans-serif",transition:"all 0.2s"}}>
          <span style={{fontSize:28,flexShrink:0}}>{p.emoji}</span>
          <div style={{textAlign:"left",flex:1}}><p style={{fontSize:14,fontWeight:700,color:sel?W.cream:W.mid}}>{p.name}</p><p style={{fontSize:14,color:W.soft,marginTop:2,fontStyle:"italic"}}>{p.react("Starbucks","38").slice(0,60)}...</p></div>
    ...
    <div key={6} style={{minHeight:"100vh",background:W.bg,padding:"60px 24px 100px",fontFamily:"'Nunito',sans-serif",textAlign:"center"}}>
      <Confetti active={true}/>
      <h2 style={{fontSize:32,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.accent,marginBottom:4}}>Welcome, {user.name}.</h2>
      ...
      <div style={{...c({marginBottom:14,textAlign:"left"})}}>
        <p style={{fontSize:13,color:W.soft,letterSpacing:1.5,textTransform:"uppercase",marginBottom:12}}>Your Setup</p>
        {[
          ["Money habit",habitLabel||"Building habits"],
          ["Goals",`${selectedGoals.length} selected`],
          ["Weekly commitment",`$${weeklyCommitment||10}/week`],
          ...
          <div key={k} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid rgba(253,240,232,0.06)`}}>
            <span style={{fontSize:16,color:W.mid}}>{k}</span>
            <span style={{fontSize:16,fontWeight:700,color:k==="Weekly commitment"?"#6EE7B7":W.cream}}>{v}</span>
          </div>
        ))}
      </div>
      <div style={{...c({background:"rgba(232,149,106,0.06)",marginBottom:24,display:"flex",gap:10,alignItems:"flex-start",textAlign:"left"})}}>
        ...
        <div><p style={{fontSize:13,fontWeight:800,color:W.accent,marginBottom:3}}>RETIREDRICHER</p><p style={{fontSize:14,color:W.mid,lineHeight:1.6,fontStyle:"italic"}}>"Know where you are. Know where you want to be. Close the gap. You just took the first step."</p></div>
      </div>
      <button onClick={next} style={btn({fontSize:18,padding:"18px"})}>Let us log your first expense →</button>
      <p style={{fontSize:13,color:W.soft,marginTop:10}}>We will walk you through it together</p>
    </div>,

    // 7 — Recording tutorial
    <div key={7} style={{minHeight:"100vh",background:W.bg,padding:"60px 24px 100px",fontFamily:"'Nunito',sans-serif"}}>
      <h2 style={{fontSize:28,fontWeight:300,fontFamily:"'Fraunces',serif",fontStyle:"italic",color:W.cream,marginBottom:8}}>Let us log your first expense</h2>
      <p style={{fontSize:16,color:W.soft,marginBottom:24,lineHeight:1.6}}>It takes 5 seconds. Here is how:</p>
      {[
        ...
        {step:"3",title:"Confirm and save",desc:"We will parse it automatically — just tap Save",emoji:"✓"},
      ].map(s=>(
        <div key={s.step} style={{...c({marginBottom:10,display:"flex",gap:14,alignItems:"center"})}}>
          ...
          <div style={{flex:1}}><p style={{fontSize:14,fontWeight:700,color:W.cream,marginBottom:2}}>{s.title}</p><p style={{fontSize:14,color:W.soft,fontStyle:"italic"}}>{s.desc}</p></div>
      ...
      <div style={{...c({background:"rgba(244,169,184,0.1)",borderColor:"rgba(244,169,184,0.2)",marginTop:16,marginBottom:20,display:"flex",gap:10,alignItems:"flex-start"})}}>
        ...
        <div><p style={{fontSize:13,fontWeight:800,color:"#F4A9B8",marginBottom:3}}>{selectedPersona?.name||"Nana Rose"}</p><p style={{fontSize:16,color:W.mid,lineHeight:1.6}}>I am right here with you. Just press and hold that button and tell me what you spent!</p></div>
      </div>
      ...
      <div style={{textAlign:"center",marginBottom:16}}>
        ...
        <p style={{marginTop:10,fontSize:14,color:W.soft}}>{recording?"Listening... release to stop":"Hold to speak"}</p>
      </div>
      <button onClick={demoEntry} style={{width:"100%",padding:"11px",background:"transparent",border:`1px dashed ${W.border}`,borderRadius:12,color:W.soft,fontSize:14,cursor:"pointer",marginBottom:16}}>Or tap here for a demo entry</button>

      {transcript&&(
        <div style={{...c({marginBottom:14})}}>
          <p style={{fontSize:13,color:W.soft,marginBottom:6,textTransform:"uppercase",letterSpacing:1}}>Heard</p>
      ...
      {firstExpense&&(
        <div style={{...c({marginBottom:14,background:"rgba(110,231,183,0.08)",borderColor:"rgba(110,231,183,0.2)"})}}>
          <p style={{fontSize:13,color:"#6EE7B7",marginBottom:8,fontWeight:700}}>Parsed successfully!</p>
          <p style={{fontSize:20,fontWeight:800,fontFamily:"'Fraunces',serif",marginBottom:8}}>${firstExpense.amount?.toFixed(2)}</p>
          <p style={{fontSize:16,color:W.mid}}>{firstExpense.store} · {firstExpense.purpose}</p>
          <button onClick={()=>{onComplete({goals:selectedGoals.map(id=>({id,...GOAL_OPTIONS_MAP[id],saved:0,target:goalAmounts[id]||GOAL_OPTIONS_MAP[id].presets[1]})),persona:selectedPersona||PERSONAS[0],weeklyCommitment:weeklyCommitment||10,habitLabel,firstExpense});}} style={btn({marginTop:14})}>Save and go to my dashboard →</button>
        </div>
      )}
      {!firstExpense&&<button onClick={()=>onComplete({goals:selectedGoals.map(id=>({id,...GOAL_OPTIONS_MAP[id],saved:0,target:goalAmounts[id]||GOAL_OPTIONS_MAP[id].presets[1]})),persona:selectedPersona||PERSONAS[0],weeklyCommitment:weeklyCommitment||10,habitLabel,firstExpense:null})} style={btn({background:"transparent",border:`1px solid ${W.border}`,color:W.soft,fontSize:14})}>Skip for now — go to dashboard</button>}
    </div>,
  ];

  const totalScreens=isGenius?SCREENS.length-6:SCREENS.length;

  return(
    <div style={{fontFamily:"'Nunito',sans-serif",color:W.cream,background:W.bg,minHeight:"100vh"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Fraunces:ital,wght@0,300;0,600;1,400&family=Nunito:wght@400;600;700;800&display=swap');@keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-5px)}}@keyframes bounceIn{0%{transform:scale(0)}60%{transform:scale(1.15)}100%{transform:scale(1)}}@keyframes cfFall{to{transform:translateY(110vh) rotate(540deg);opacity:0}}*{box-sizing:border-box;margin:0;padding:0}button,input{font-family:'Nunito',sans-serif}::-webkit-scrollbar{display:none}body{background:#0A0500;margin:0}`}</style>
      {SCREENS[screen]}
      {screen>0&&<button onClick={back} style={{position:"fixed",top:20,left:20,background:"none",border:"none",color:W.soft,cursor:"pointer",fontSize:16,fontFamily:"'Nunito',sans-serif",zIndex:100}}>← Back</button>}
      <div style={{position:"fixed",bottom:20,left:"50%",transform:"translateX(-50%)",display:"flex",gap:4,zIndex:100}}>
        {SCREENS.map((_,i)=><div key={i} style={{width:i===screen?20:6,height:8,borderRadius:4,background:i===screen?W.accent:"rgba(253,240,232,0.15)",transition:"all 0.3s"}}/>)}
      </div>
    </div>
  );
}
...
export default function App(){
  const [authState,setAuthState]=useState(()=>{
    const saved=loadUser();
    return saved&&saved.onboarded?"app":"login";
  });
  ...
  const checkBadge=(id)=>{
    if(!earnedBadges.includes(id)){
      ...
      if(badge){setEarnedBadges(p=>[...p,id]);setActiveBadge(badge);}
    }
  };
  ...
  const save=()=>{
    ...
    setExpenses(updated);
    setSaving(true);
    setReaction(persona.save());
    if(expenses.length===0)checkBadge("first_log");
  ...
  if(authState==="login") return(
    <Login
      onLogin={(saved)=>{setUser(saved);setGoals(saved.goals||[]);setAuthState(saved.onboarded?"app":"onboard");}}
      onRegister={(data)=>{const newUser={...data,onboarded:false};saveUser(newUser);setUser(newUser);setAuthState("onboard");}}
    />
  );

  if(authState==="onboard") return(
    <Onboarding user={user} onComplete={(data)=>{
  ...
  return(
    <div style={{minHeight:"100vh",background:W.bg,fontFamily:"'Nunito',sans-serif",color:W.text,maxWidth:430,margin:"0 auto"}}>
      ...
      {showWalkAway&&<WalkAwayResult persona={persona} onClose={()=>{setShowWalkAway(false);setActiveBodyguard(null);setResistanceStreak(s=>s+1);checkBadge("first_resist");}}/>}

      {/* HEADER */}
      <div style={{padding:"44px 20px 14px",display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
        <div>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:2}}><Sparkle size={14}/><p style={{fontSize:13,color:W.soft,letterSpacing:2,textTransform:"uppercase"}}>{monthLabel()}</p></div>
          <h1 style={{fontSize:22,fontWeight:800,fontFamily:"'Fraunces',serif",fontStyle:"italic"}}>Rich Girl Habits</h1>
          <p style={{fontSize:13,color:W.soft}}>A RetireRicher product</p>
        </div>
        <div style={{textAlign:"right"}}>
          <div style={{display:"flex",alignItems:"center",gap:5,justifyContent:"flex-end",marginBottom:2}}><span style={{fontSize:16}}>{persona.emoji}</span><span style={{fontSize:13,color:W.soft}}>{persona.name}</span></div>
          <p style={{fontSize:18,fontWeight:800,color:W.accent,fontFamily:"'Fraunces',serif"}}>${totalSaved.toLocaleString()}</p>
          <p style={{fontSize:13,color:W.soft}}>total saved</p>
        </div>
      </div>

      {/* TABS */}
      <div style={{display:"flex",gap:3,padding:"0 20px 14px"}}>
        {NAV.map(n=><button key={n.id} onClick={()=>setTab(n.id)} style={{flex:1,padding:"9px 4px",borderRadius:10,border:"none",cursor:"pointer",fontSize:13,fontWeight:800,background:tab===n.id?W.accent:"rgba(255,255,255,0.05)",color:tab===n.id?W.bg:W.soft,transition:"all 0.2s"}}><div style={{fontSize:14,marginBottom:2}}>{n.emoji}</div>{n.label}</button>)}
      </div>
        ...
        <div style={{padding:"0 20px 120px",animation:"scaleIn 0.35s ease"}}>
          {reaction&&(
            <div style={{...card({background:"rgba(244,169,184,0.1)",borderColor:"rgba(244,169,184,0.25)",marginBottom:14,display:"flex",gap:10,alignItems:"flex-start"})}}>
              ...
              <div style={{flex:1}}><p style={{fontSize:13,fontWeight:800,color:persona.color,marginBottom:3}}>{persona.name}</p><p style={{fontSize:16,color:W.mid,lineHeight:1.6}}>{reaction}</p></div>
          ...
          <div style={{...card({marginBottom:14})}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
              <div><p style={{fontSize:13,color:W.soft,marginBottom:3,textTransform:"uppercase",letterSpacing:1}}>Spent This Month</p><p style={{fontSize:32,fontWeight:800,fontFamily:"'Fraunces',serif"}}>${totalSpent.toFixed(0)}</p><p style={{fontSize:13,color:W.soft}}>of ${totalBudget} budget</p></div>
              <div style={{textAlign:"right"}}><p style={{fontSize:13,color:W.soft,marginBottom:3,textTransform:"uppercase",letterSpacing:1}}>Total Saved</p><p style={{fontSize:24,fontWeight:800,color:W.accent}}>${totalSaved.toLocaleString()}</p><p style={{fontSize:13,color:W.soft}}>since day 1</p></div>
            </div>
            ...
            {totalSpent===0&&<p style={{fontSize:13,color:W.soft,marginTop:8,fontStyle:"italic",textAlign:"center"}}>No expenses logged yet — tap Record to start</p>}
          </div>

          {/* Budget Bodyguard */}
          <div style={{...card({background:"rgba(239,68,68,0.06)",borderColor:"rgba(239,68,68,0.2)",marginBottom:14})}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:8,height:8,borderRadius:"50%",background:"#EF4444",animation:"blink 1.5s infinite"}}/><p style={{fontSize:13,fontWeight:800,color:"#FCA5A5",letterSpacing:1}}>BUDGET BODYGUARD</p></div>
              <p style={{fontSize:13,color:W.soft}}>{resistanceStreak} resistances</p>
            </div>
            <div style={{display:"flex",gap:8}}>
              {[{id:"starbucks",name:"Starbucks",emoji:"☕",type:"trap"},{id:"target",name:"Target",emoji:"🎯",type:"caution"},{id:"amazon",name:"Amazon",emoji:"📦",type:"trap"}].map(z=>(
                <button key={z.id} onClick={()=>setActiveBodyguard(z)} style={{flex:1,padding:"10px 6px",borderRadius:12,border:`1px solid ${z.type==="trap"?"rgba(239,68,68,0.3)":"rgba(251,191,36,0.3)"}`,background:z.type==="trap"?"rgba(239,68,68,0.06)":"rgba(251,191,36,0.06)",color:W.mid,fontSize:13,fontWeight:700,cursor:"pointer",textAlign:"center"}}><div style={{fontSize:20,marginBottom:4}}>{z.emoji}</div>{z.name}</button>
              ))}
            </div>
          </div>

          <button onClick={()=>setTab("record")} style={{width:"100%",padding:"14px",background:"transparent",border:`1.5px dashed ${W.border}`,borderRadius:14,color:W.mid,fontSize:16,fontWeight:700,cursor:"pointer",marginBottom:14,display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
            <span style={{fontSize:22,animation:"float 2s ease-in-out infinite"}}>🎙️</span> Tap to record an expense
          </button>

          <p style={{fontSize:13,color:W.soft,letterSpacing:1.5,textTransform:"uppercase",marginBottom:10}}>Your Goals</p>
          {goals.length===0?(
            <div style={{...card({textAlign:"center",padding:"24px 20px"})}}>
              <p style={{fontSize:32,marginBottom:8}}>🎯</p>
              <p style={{fontSize:16,color:W.soft}}>Complete setup to see your goals here</p>
            </div>
          ):goals.map(g=>(
            <div key={g.id} style={{...card({marginBottom:10})}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:20}}>{g.emoji}</span><div><p style={{fontSize:16,fontWeight:700}}>{g.label}</p><p style={{fontSize:13,color:W.soft}}>${((g.target||0)-(g.saved||0)).toLocaleString()} to go</p></div></div>
                <div style={{textAlign:"right"}}><p style={{fontSize:16,fontWeight:800,color:g.color||W.accent}}>${(g.saved||0).toLocaleString()}</p><p style={{fontSize:13,color:W.soft}}>of ${(g.target||0).toLocaleString()}</p></div>
              </div>
              <div style={{height:5,background:"rgba(253,240,232,0.08)",borderRadius:3}}><div style={{height:"100%",borderRadius:3,background:g.color||W.accent,width:`${((g.saved||0)/(g.target||1))*100}%`,transition:"width 0.6s ease"}}/></div>
            </div>
          ))}

          {/* Sign out */}
          <button onClick={()=>{setAuthState("login");}} style={{width:"100%",padding:"10px",background:"transparent",border:`1px solid rgba(253,240,232,0.08)`,borderRadius:10,color:W.soft,fontSize:13,cursor:"pointer",marginTop:8}}>Sign out</button>
        </div>
      )}
      ...
      {tab==="record"&&(
        <div style={{padding:"0 20px 120px",animation:"scaleIn 0.35s ease"}}>
          ...
          <div style={{...card({padding:"32px 20px",textAlign:"center",marginBottom:14})}}>
            ...
            <p style={{marginTop:14,fontSize:16,color:W.mid}}>{recording?"Listening... release to stop":"Hold to record an expense"}</p>
            <p style={{marginTop:4,fontSize:13,color:W.soft}}>"I spent $38.50 at Starbucks for coffee"</p>
          </div>
          <button onClick={demo} style={{width:"100%",padding:"11px",background:"transparent",border:`1px dashed ${W.border}`,borderRadius:12,color:W.soft,fontSize:14,cursor:"pointer",marginBottom:14}}>Try a demo entry</button>
          {transcript&&<div style={{...card({marginBottom:14})}}><p style={{fontSize:13,color:W.soft,marginBottom:6,textTransform:"uppercase",letterSpacing:1}}>Heard</p><p style={{fontSize:14,fontStyle:"italic",color:W.mid}}>"{transcript}"</p></div>}
          {parsed&&!saving&&(
            <div style={{...card({marginBottom:14})}}>
              ...
              {[["Store",parsed.store],["Category",CATS[parsed.category]?.label],["Purpose",parsed.purpose]].map(([k,v])=>(
                <div key={k} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid rgba(253,240,232,0.06)"}}><span style={{fontSize:14,color:W.soft}}>{k}</span><span style={{fontSize:16,fontWeight:600}}>{v}</span></div>
              ))}
              <div style={{marginTop:14}}>
                <p style={{fontSize:13,fontWeight:700,color:W.mid,marginBottom:8}}>What are you saving for?</p>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  {goals.map(g=><div key={g.id} style={{padding:"5px 10px",background:W.aLight,border:`1px solid ${W.border}`,borderRadius:99,fontSize:13,color:W.cream,fontWeight:700,cursor:"pointer"}}>{g.emoji} {g.label.split(" ")[0]}</div>)}
                </div>
              </div>
      ...
      {tab==="growth"&&(
        <div style={{padding:"0 20px 120px",animation:"scaleIn 0.35s ease"}}>
          <div style={{...card({background:"rgba(232,149,106,0.08)",borderColor:`${W.accent}33`,marginBottom:16,padding:"24px 20px",textAlign:"center"})}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:6}}><Sparkle size={14}/><p style={{fontSize:13,color:W.soft,letterSpacing:2,textTransform:"uppercase"}}>Total Saved Since Day 1</p><Sparkle size={14}/></div>
          ...
          <div style={{...card({marginBottom:14})}}>
            <p style={{fontSize:13,color:W.soft,letterSpacing:1.5,textTransform:"uppercase",marginBottom:4}}>The Gut Punch Calculator</p>
            <h2 style={{fontSize:20,fontWeight:800,fontFamily:"'Fraunces',serif",fontStyle:"italic",marginBottom:16,color:W.cream,lineHeight:1.3}}>What is your Starbucks habit costing you?</h2>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div style={{textAlign:"center"}}><p style={{fontSize:13,color:W.soft,marginBottom:4}}>RIGHT NOW</p><p style={{fontSize:28,fontWeight:800,color:"#FCA5A5"}}>8x</p><p style={{fontSize:13,color:W.soft}}>per week</p></div>
              <div style={{color:W.soft,fontSize:20}}>→</div>
              <div style={{textAlign:"center"}}>
                <p style={{fontSize:13,color:W.soft,marginBottom:4}}>YOUR GOAL</p>
                ...
                <p style={{fontSize:13,color:W.soft}}>per week</p>
              </div>
              <div style={{textAlign:"center"}}><p style={{fontSize:13,color:W.soft,marginBottom:4}}>SAVED</p><p style={{fontSize:28,fontWeight:800,color:"#6EE7B7"}}>${weeklySaved.toFixed(0)}</p><p style={{fontSize:13,color:W.soft}}>per week</p></div>
            </div>
            {[[1,y1],[5,y5],[10,y10],[30,y30]].map(([yr,val])=>(
              <div key={yr} style={{marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:16,color:W.mid}}>{yr} year{yr>1?"s":""}</span><span style={{fontSize:16,fontWeight:800,color:yr>=10?W.accent:W.cream}}>{formatK(val)}</span></div>
            ...
            <div style={{marginTop:14,padding:"14px",background:"rgba(232,149,106,0.12)",borderRadius:12,border:"1px solid rgba(232,149,106,0.25)",textAlign:"center"}}>
              <Sparkle size={16}/>
              <p style={{fontSize:16,color:W.mid,marginBottom:4,marginTop:8}}>In 5 years that is</p>
              <p style={{fontSize:28,fontWeight:800,fontFamily:"'Fraunces',serif",color:W.accent,marginBottom:6}}>{formatK(y5)}</p>
              <p style={{fontSize:16,color:W.cream,fontStyle:"italic",fontFamily:"'Fraunces',serif"}}>"Your Emergency Fund. Done."</p>
              <p style={{fontSize:13,color:W.soft,marginTop:8,fontStyle:"italic"}}>Your future self wants you to.</p>
            </div>
          </div>

          <p style={{fontSize:13,color:W.soft,letterSpacing:1.5,textTransform:"uppercase",marginBottom:10}}>Your Badges</p>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:14}}>
            {BADGES.map(b=>{
              ...
              return(<div key={b.id} onClick={()=>earned&&setActiveBadge(b)} style={{background:earned?"rgba(196,150,90,0.1)":"rgba(255,255,255,0.03)",border:`1px solid ${earned?"rgba(196,150,90,0.3)":"rgba(255,255,255,0.06)"}`,borderRadius:14,padding:"14px 10px",textAlign:"center",cursor:earned?"pointer":"default",opacity:earned?1:0.4}}>
                ...
                <p style={{fontSize:13,fontWeight:700,color:earned?tc:"rgba(253,240,232,0.3)",lineHeight:1.3}}>{b.name}</p>
          ...
          <div style={{...card({background:"rgba(232,149,106,0.06)",display:"flex",gap:10})}}>
            ...
            <div><p style={{fontSize:14,fontWeight:800,color:W.accent,marginBottom:4}}>RetireRicher</p><p style={{fontSize:14,color:W.mid,lineHeight:1.65,fontStyle:"italic"}}>"Know where you are. Know where you want to be. Identify the gap. That is the whole game — and you are playing it."</p></div>
          </div>
        </div>
      )}
      ...
      {tab==="history"&&(
        <div style={{padding:"0 20px 120px",animation:"scaleIn 0.35s ease"}}>
          <div style={{...card({marginBottom:14,display:"flex",justifyContent:"space-between"})}}>
            <div><p style={{fontSize:13,color:W.soft,marginBottom:3}}>THIS MONTH</p><p style={{fontSize:26,fontWeight:800,fontFamily:"'Fraunces',serif"}}>${totalSpent.toFixed(0)}</p></div>
            <div style={{textAlign:"right"}}><p style={{fontSize:13,color:W.soft,marginBottom:3}}>TRANSACTIONS</p><p style={{fontSize:26,fontWeight:800,fontFamily:"'Fraunces',serif"}}>{thisMonth.length}</p></div>
          </div>
          {Object.keys(byCat).length>0&&(
            <div style={{...card({marginBottom:14})}}>
              <p style={{fontSize:13,color:W.soft,letterSpacing:1.5,textTransform:"uppercase",marginBottom:12}}>By Category</p>
              {Object.entries(byCat).sort(([,a],[,b])=>b-a).map(([cat,spent])=>{
                ...
                return(<div key={cat} style={{marginBottom:10}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:12}}>{CATS[cat]?.emoji} {CATS[cat]?.label}</span><span style={{fontSize:16,fontWeight:700,color:over?"#FCA5A5":W.text}}>${spent.toFixed(0)}</span></div>
          ...
          {expenses.length===0?(
            <div style={{...card({textAlign:"center",padding:"32px 20px"})}}>
              ...
              <p style={{fontSize:14,color:W.soft,marginBottom:16}}>Tap Record to log your first expense</p>
              <button onClick={()=>setTab("record")} style={{padding:"12px 24px",background:W.accent,border:"none",borderRadius:12,color:W.bg,fontSize:18,fontWeight:800,cursor:"pointer"}}>Start Recording →</button>
            </div>
            ...
            <div key={e.id} style={{...card({display:"flex",alignItems:"center",gap:12,padding:"12px 14px",marginBottom:8})}}>
              ...
              <div style={{flex:1,minWidth:0}}><p style={{fontSize:16,fontWeight:700,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{e.store}</p><p style={{fontSize:13,color:W.soft,marginTop:1}}>{e.member} · {e.purpose}</p></div>
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:"rgba(10,5,0,0.96)",borderTop:`1px solid ${W.border}`,backdropFilter:"blur(20px)",padding:"10px 20px 24px"}}>
        <div style={{display:"flex",gap:3}}>
          {NAV.map(n=><button key={n.id} onClick={()=>setTab(n.id)} style={{flex:1,padding:"9px 4px",borderRadius:10,border:"none",cursor:"pointer",fontSize:13,fontWeight:800,background:tab===n.id?W.accent:"transparent",color:tab===n.id?W.bg:W.soft,transition:"all 0.2s"}}><div style={{fontSize:16,marginBottom:2}}>{n.emoji}</div>{n.label}</button>)}
        </div>
      </div>
    </div>
  );
