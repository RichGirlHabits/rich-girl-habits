...
export default function App() {
  ...
  const floralScreen = (children) => (
    <div style={{
      ...
      {/* dark overlay so text is readable over the floral */}
      <div style={{ position:"absolute", inset:0, background:"rgba(10,5,0,0.52)", zIndex:0 }}/>
  ...
  // ── WELCOME SCREEN ──────────────────────────────────────────────────────────
  if (screen === "welcome") return floralScreen(
    <>
      <style>{CSS}</style>
      <div style={{ fontSize:72, marginBottom:16, animation:"float 2s ease-in-out infinite" }}>🌸</div>
      <h1 style={{ fontSize:40, fontWeight:300, fontFamily:"'Fraunces',serif", fontStyle:"italic", color:"#FDF0E8", marginBottom:6, lineHeight:1.2, textShadow:"0 2px 12px rgba(0,0,0,0.5)" }}>
        Rich Girl Habits
      </h1>
      <p style={{ fontSize:11, color:"rgba(253,240,232,0.7)", marginBottom:8, letterSpacing:2, textTransform:"uppercase" }}>A RetireRicher Product</p>
      <div style={{ width:60, height:1, background:`linear-gradient(90deg,transparent,#E8956A,transparent)`, margin:"12px auto 16px" }}/>
      <p style={{ fontSize:17, color:"rgba(253,240,232,0.9)", fontStyle:"italic", fontFamily:"'Fraunces',serif", fontWeight:300, marginBottom:8, lineHeight:1.6, textShadow:"0 1px 8px rgba(0,0,0,0.6)" }}>
        "Your future self wants you to."
      </p>
      <p style={{ fontSize:13, color:"rgba(253,240,232,0.6)", marginBottom:48, fontStyle:"italic" }}>
        For when you have more month than money.
      </p>
      <button onClick={()=>setScreen("app")} style={{ width:"100%", padding:"18px", background:"#E8956A", border:"none", borderRadius:16, color:"#0A0500", fontSize:16, fontWeight:800, cursor:"pointer", marginBottom:14, letterSpacing:0.3 }}>
        Get Started 🌸
      </button>
      <button onClick={()=>setScreen("app")} style={{ width:"100%", padding:"14px", background:"rgba(253,240,232,0.1)", border:"1px solid rgba(253,240,232,0.25)", borderRadius:16, color:"rgba(253,240,232,0.6)", fontSize:13, cursor:"pointer" }}>
        I already have an account
      </button>
      <p style={{ fontSize:11, color:"rgba(253,240,232,0.25)", marginTop:32, lineHeight:1.6 }}>
